...
class TagQuerySet(models.QuerySet):
    def popular(self):
        prefetch = models.Prefetch("posts")
        return (
            self.prefetch_related(prefetch)
            .annotate(models.Count("posts"))
...
class PostQuerySet(models.QuerySet):
    ...
    def fetch_with_comments_count(self):
        posts_comments_counted = Post.objects.filter(id__in=self).annotate(
            models.Count("comments")
        )
        comments_counts = dict(
            posts_comments_counted.values_list("pk", "comments__count")
        )
        for post in self:
            post.comments_count = comments_counts[post.pk]
        return self
